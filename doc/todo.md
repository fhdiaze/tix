...
- [x] **Present via `SetDIBitsToDevice`.** Blit the used `width × height`
...
- [x] **Wire up resize (no per-`WM_SIZE` realloc).** The `WM_SIZE` case in
      ...
      render thread only.
- [x] **Sanity content.** Fill with a solid color / gradient to prove writes
      land right-side up and right-sized.
- [x] **Drop the `TextOutA` path.** Remove `window_render_lines`'
      GDI-text-drawing once the backbuffer present works.
- [x] Exit check: resize repaints correctly — no flicker, no artifacts.
